...
public class SquirrelResultSetReader {
    private final static ILogger s_log = LoggerController.createLogger(SquirrelResultSetReader.class);

    private static final StringManager s_stringMgr = StringManagerFactory
            .getStringManager(SquirrelResultSetReader.class);
    ...
    /**
     * The indices into the <TT>ResultSet that we want to read, starting from 1 (not
     * 0). If this contains {1, 5, 6} then only columns 1, 5, and 6 will be read. If
     * <TT>null or empty then all columns are read.
     */
    private final int[] _columnIndices;

    /**
     * The number of columns to read. This may or may not be the same as the number
     * of columns in the <TT>ResultSet</TT>. @see _columnIndices.
     */
     ...
     * @param dialectType the DialectType
     * @throws SQLException
     */
    public SquirrelResultSetReader(ResultSet rs, DialectType dialectType) throws SQLException {
        this(rs, null, dialectType);
    }

    public SquirrelResultSetReader(ResultSetWrapper rs, DialectType dialectType) throws SQLException {
        this(rs, null, dialectType);
    }
     ...
     * @throws SQLException
     */
    public SquirrelResultSetReader(ResultSet rs, int[] columnIndices, DialectType dialectType) throws SQLException {
        this(new ResultSetWrapper(rs), columnIndices, dialectType);
    }

    public SquirrelResultSetReader(ResultSetWrapper rs, int[] columnIndices, DialectType dialectType)
            throws SQLException {
        if (rs == null) {
     ...
     * returned where each element of the array is an object representing the
     * contents of the column. These objects could be of type <TT>String</TT>,
     * <TT>BigDecimal</TT> etc.
     *
     * <P>
     * If an error occurs calling <TT>next()</TT> on the <TT>ResultSet</TT> then an
     * <TT>SQLException will be thrown, however if an error occurs
    ...
    /**
     * Read the next row from the <TT>ResultSet</TT> for use in the ContentTab. This
     * is different from readRow() in that data is put into the Object array in a
     * form controlled by the DataType objects, and may be used for editing the data
     * and updating the DB. If no more rows then <TT>null</TT> will be returned,
     * otherwise an <TT>Object[]</TT> will be returned where each element of the
     * array is an object representing the contents of the column. These objects
     * could be of type <TT>String</TT>, <TT>BigDecimal</TT> etc.
     *
     * <P>
     * If an error occurs calling <TT>next()</TT> on the <TT>ResultSet</TT> then an
     * <TT>SQLException will be thrown, however if an error occurs
     ...
     * @throws SQLException Error occurred on <TT>ResultSet.next()</TT>.
     */
    public Object[] readRow(ColumnDisplayDefinition colDefs[], BlockMode blockMode, boolean limitRead)
            throws SQLException {
        _errorOccured = false;
        if (_rs.next(blockMode)) {
     ...
     *
     * @param idx the column index to get the type name for.
     * @return the type name of the column, or the Java class, or "Unavailable" if
    ...
    private String safelyGetColumnTypeName(int idx) {
        ...
        try {
            /*
             * Fails on DB2 8.1 for Linux However, Windows 8.1 fixpak 14 driver (2.10.52)
             * works without exception Also, Linux 9.0.1 server with 3.1.57 driver works
             * fine as well
             */
            columnTypeName = _rsmd.getColumnTypeName(idx);
        } catch (SQLException e) {
            if (s_log.isInfoEnabled()) {
                s_log.info("doRead: ResultSetMetaData.getColumnTypeName(" + idx + ") threw an unexpected exception - "
                        + e.getMessage());
                s_log.info("Unable to determine column type name so " + "any custom types provided by plugins will be "
        ...
        if (columnTypeName == null) {
            try {
                ...
                if (s_log.isInfoEnabled()) {
                    s_log.info("doRead: ResultSetMetaData.getColumnClassName(" + idx
    ...
    private Object doSqlRead(int sqlType, String sqlTypeName, int index) throws SQLException {
        ResultSet rs = _rs.getResultSet();
        switch (sqlType) {
            case Types.NULL:
            ...
            case Types.DECIMAL:
            case Types.NUMERIC:
                return readNumeric(index);

            case Types.INTEGER:
            case Types.SMALLINT:
            case Types.TINYINT:
                return readInt(index, sqlTypeName);
            ...
            // TODO: ResultSet has it's own NCLOB support (rs.getNClob(i)). It is probably
            // not valid to
            // call getClob on an NClob column ?? So, may need to implement new
            // DataTypeNClob type
            // component (see below):
            //
            // case Types.NCLOB:
            // return DataTypeNClob.staticReadResultSet(_rs, index);

            case Types.JAVA_OBJECT:
                return readObject(index);

            case Types.OTHER:
                return readOther(index);

            default:
                Integer colTypeInteger = Integer.valueOf(sqlTypeName);
                return s_stringMgr.getString("SquirrelResultSetReader.unknown", colTypeInteger);
        }
    }

    /**
     * Method used to read data for all Tabs except the ContentsTab, where the data
     * is used only for reading. The only data read in the non-ContentsTab tabs is
     * Meta-data about the DB, which means that there should be no BLOBs, CLOBs, or
     * unknown fields.
     */
    private Object[] doRead() {
        ...
        for (int i = 0; i < _columnCount && !_stopExecution; ++i) {
            ...
            try {
                ...
                /*
                 * See if there is a plugin-registered DataTypeComponent that can handle this
                 * column.
                 */
                row[i] = CellComponentFactory.readResultWithPluginRegisteredDataType(_rs.getResultSet(), columnType,
                        columnTypeName, idx, _dialectType);
                if (row[i] == null) {
                    row[i] = doSqlRead(columnType, columnTypeName, idx);
                }
            } catch (Throwable th) {
                ...
                if (!_stopExecution) {
                    _errorOccured = true;
                    row[i] = s_stringMgr.getString("SquirrelResultSetReader.error");
    ...
    private Object readOther(int columnIdx) throws SQLException {
        ...
        // ?? else
        // ?? {
        return s_stringMgr.getString("SquirrelResultSetReader.other");
        // ?? }
     ...
     * @param columnTypeName the type name of the column according to the
     *                       ResultSetMetaData
    ...
    private Object readInt(int columnIdx, String columnTypeName) throws SQLException {
         ...
         *
         * If we are working with a signed integer, then it should be ok to store in a
         * Java integer which is always signed. However, if we are working with an
         * unsigned integer type, Java doesn't have this so use a long instead. Or if
         * the type of the object is Long then use that instead of Integer.
         */
    ...
    /**
     * Method used to read data for the ContentsTab, where the data is used for both
     * reading and editing.
     *
