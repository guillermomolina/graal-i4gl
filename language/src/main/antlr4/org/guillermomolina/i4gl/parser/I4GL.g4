...
returnStatement
   : RETURN variableOrConstantList? EOL
   ;
...
runStatement
   : RUN (variable | string) (IN FORM MODE | IN LINE MODE)? (WITHOUT WAITING | RETURNING variable)? EOL
   ;
...
procedureStatement
   : CALL procedureIdentifier (LPAREN (actualParameter (COMMA actualParameter)*)? RPAREN)? (RETURNING variable (COMMA variable)*)? EOL
   ;
...
logicalFactor
   ...
   | (sqlExpression NOT? LIKE) sqlExpression NOT? LIKE sqlExpression (ESCAPE CHAR_LITERAL)?
...
sqlLiteral
   : numericConstant | string | NULL | FALSE | TRUE
   ;
...
forStatement
   : FOR controlVariable EQUAL initialValue TO finalValue (STEP numericConstant)? EOL codeBlock? END FOR EOL
   ;
...
forEachStatement
   : FOREACH identifier (USING variableList)? (INTO variableList)? (WITH REOPTIMIZATION)? EOL codeBlock? END FOREACH
   ;
...
caseStatement1
   : CASE expression EOL (WHEN expression EOL codeBlock)+ (OTHERWISE EOL codeBlock)? END CASE EOL
   ;

caseStatement2
   : CASE EOL (WHEN ifCondition EOL codeBlock)+ (OTHERWISE EOL codeBlock)? END CASE EOL
   ;
...
reportStatement
   ...
   | TERMINATE REPORT constantIdentifier EOL
   | FINISH REPORT constantIdentifier EOL
   | PAUSE string? EOL
   | NEED expression LINES EOL
   | PRINT (printExpressionList SEMI? | FILE string)? EOL
   | SKIP2 (expression (LINE | LINES) | TO TOP OF PAGE) EOL
   | OUTPUT TO REPORT constantIdentifier LPAREN (expression (COMMA expression)*)? RPAREN EOL
   ;
...
constructStatement
   : CONSTRUCT (BY NAME variable ON columnsList | variable ON columnsList FROM fieldList) attributeList? (HELP numericConstant)? (constructGroupStatement+ END CONSTRUCT)? EOL
   ;
...
promptStatement
   : PROMPT expression (COMMA expression)* attributeList? FOR CHAR? variable (HELP numericConstant)? attributeList? ((ON KEY LPAREN keyList RPAREN codeBlock?)* END PROMPT)? EOL
   ;
...
inputStatement
   : INPUT (BY NAME expression (COMMA expression)* (WITHOUT DEFAULTS)? | expression (COMMA expression)* (WITHOUT DEFAULTS)? FROM fieldList) attributeList? (HELP numericConstant)? (inputGroupStatement+ END INPUT)? EOL
   ;
...
screenStatement
   ...
   | inputArrayStatement
   | menuStatement
   | OPEN FORM expression FROM expression EOL
   | OPEN WINDOW expression AT expression COMMA expression (WITH FORM expression | WITH expression ROWS COMMA expression COLUMNS) windowAttributeList? EOL
   | optionsStatement
   | SCROLL fieldList (COMMA fieldList)* (UP | DOWN) (BY numericConstant)? EOL
   ;
...
sqlLoadStatement
   : LOAD FROM (variable | string) (DELIMITER (variable | string))? (INSERT INTO tableIdentifier (LPAREN columnsList RPAREN)? | sqlInsertStatement)
   ;
...
sqlDeleteStatement
   : DELETE FROM tableIdentifier (WHERE (condition | CURRENT OF cursorName))?
   ;
...
databaseDeclaration
   : DATABASE (constantIdentifier (ATSYMBOL constantIdentifier)?) EXCLUSIVE? SEMI? EOL
   ;
...
wheneverStatement
   : WHENEVER wheneverType wheneverFlow
   ;
